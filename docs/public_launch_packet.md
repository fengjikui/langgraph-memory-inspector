...
real bug patterns, and do not make stars the main call to action.
...
```text
The demo reproduces a stale-memory bug: the user moves from Shanghai to Hangzhou, but the agent still answers with Shanghai context. The checkpoint store has the evidence; the inspector connects final answer -> diagnostic -> checkpoint -> write chain.
```
...
Demo bug: a user moves from Shanghai to Hangzhou, but the agent still answers with Shanghai context. LangGraph Memory Inspector reads the checkpoint store locally and connects final answer -> diagnostic -> checkpoint -> write evidence.
...
- timeline, diff, writes, diagnostics
- causal chain view
...
- causal chain：从诊断回到相关 checkpoint/write/node
...
The demo is a concrete memory bug: a user moves from Shanghai to Hangzhou, but the agent still answers using Shanghai context. The checkpoint DB has the evidence. The inspector shows the timeline, state diff, writes, deterministic diagnostics, and a causal chain from diagnostic to checkpoint/write evidence.
